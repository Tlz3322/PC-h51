...
const second = computed(() => {
  return datailData.order_start_time ? datailData.order_start_time + 7200000 - Date.now() : 0
})
...
onMounted(async () => {
  ...
  Object.assign(datailData, data.data)
...
<template>
  <div class="container">
    ...
    <status-bar :item="stateMap[datailData].trade_state" />
    <div class="tips">
      <div
        class="dzf"
        v-if="datailData.trade_state === '待支付'"
      >
        ...
        <div class="text3">
          <van-button
            @click="showCode = true"
            type="success"
            >立即支付(0.5)元</van-button
          >
        </div>
      </div>
      <div
        class="dzf"
        v-if="datailData.trade_state === '待服务'"
      >
      ...
      <div
        class="dzf"
        v-if="datailData.trade_state === '已完成'"
      >
      ...
      <div
        class="dzf"
        v-if="datailData.trade_state === '已取消'"
      >
